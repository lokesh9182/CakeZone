...
<%-- 					<td><a class="btext"
				...
				<c:if test="${order.status!='Order accepted'}">
					<a class="btext"
						href="/ShoppingCart/sc.com/acceptOrRejectOrder?cakeid=${ order.cakeid}&&status=428&&userid=${order.userid}"
						onclick="if(!confirm('Are You Sure That You Want To Accept This order..!!'))return false">
							<button class="mybutton">Accept</button>
					</a>
					</c:if>
					<a class="btext"
						href="/ShoppingCart/sc.com/acceptOrRejectOrder?cakeid=${ order.cakeid }&&status=824&&userid=${order.userid}"
						onclick="if(!confirm('Are You Sure That You Want To Reject This order..!!'))return false">
							<button class="mybutton">Reject </button>
					</a></td>
				</tr>
			</c:forEach>
